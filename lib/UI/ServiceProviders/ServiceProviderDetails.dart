...
class ServiceProviderDetails extends BaseUI<ServiceProvidersBloc> {
  ...
  ServiceProviderDetails({this.providerId, this.bloc}) : super(bloc: ServiceProvidersBloc());
        ...
        builder: (context, snapshot) {
          ...
          return Scaffold(
            appBar: AppBar(
              ...
              actions: [
                dataStore.user == null || snapshot.data.data.canRate == 0 ?Container():
                InkWell(
                  child: SizedBox(
                    width: 50,
                    height: 50,
                    child: Column(
                      children: [
                        Icon(Icons.star_rate , size: 25,),
                        Text(AppLocalizations.of(context).trans("rate"),style: AppTextStyle.smallWhiteBold,)
                      ],
                    ),
                  ),
                  onTap: () async {
                    //snapshot.data.data.ra
                    var res = await showDialog<double>(context: context , builder: (context){
                      double rate = snapshot.data.data.rate.toDouble();
                      return AlertDialog(
                        content: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Text(AppLocalizations.of(context).trans("rate") +" "+ snapshot.data.data.businessName),
                            SizedBox(height: 8,),
                            Directionality(
                            ...
                            SizedBox(height: 8,),
                            CustomAppButton(
                              child: Text(AppLocalizations.of(context).trans("rate") , style: AppTextStyle.mediumWhiteBold,),
                    ...
                    if(res != null){
                      ...
                      widget.bloc.addRate(
                          serviceProviderId: widget.providerId.toString(),
                          rate: res.toString(),
                          onData: (){
                            showErrorDialog(context, AppLocalizations.of(context).trans("ratedSuccessfully"),isError: false);
                        ...
                        onError: (String e){
                          showErrorDialog(context, e.isEmpty?AppLocalizations.of(context).trans("wrong"):e,isError: true);
                          setState(() {
                            _isLoading=false;
                          });
                        }
                  ...
                  children: [
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            children: [
                              Text(
                                AppLocalizations.of(context).trans("businessType"),
                    ...
                    Row(
                      children: [
                        ...
                        Flexible(child: Text(snapshot.data.data.location,
                          style: AppTextStyle.mediumWhiteBold,
                        ),)
                      ],
                    ),
                            ...
                            onTap: (){
                              Navigator.of(context).push(MaterialPageRoute(builder: (context){
                                return ServiceProviderCouponsPage(
                                  serviceProviderName: snapshot.data.data.businessName,
                                  serviceProviderId: snapshot.data.data.id.toString(),
                                  networkId: snapshot.data.data.networkId.toString(),
                                );
                              }));
                            },
                            child: Container(
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Text(AppLocalizations.of(context).trans("showCoupons"),style: AppTextStyle.mediumWhiteBold,),
                                    SizedBox(width: 16,),
                    ...
                    Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          snapshot.data.data.longitude == null ?Container():Container(
                            height: MediaQuery.of(context).size.height * 0.2,
                            width: double.infinity,
                            child: //AppleMapWidget
                          MapWidget(LatLng(double.parse(snapshot.data.data.latitude??"0.0"), double.parse(snapshot.data.data.longitude??"0.0"))),
